...
module.exports = function (Aquifer) {
  ...
  var Extension = function (name, source) {
    ...
    self.loadConfig = function () {
      ...
      self.downloaded = pathExists(self.path) ? true : false;
    };

    self.loadConfig();

    // Parse the final source value.
    self.source = source || self.config.source;
    if (!self.source) {
      Aquifer.console.log('The ' + self.name + ' extension is missing the source property. Please add it in aquifer.json.', 'error');
    }

    // Use the source value to determine whether the extension is local.
    self.isLocal = pathExists(self.source);
