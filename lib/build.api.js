  ...
  var Build = function (destination, options) {
    ...
    self.create = function (make, callback) {
      ...
      drush.init({log: true, cwd: self.destination})
        ...
        .then(buildStep('Generating settings files...', function () {
          ...
          if (dbSettings == null || _.isEmpty(dbSettings)) {
            ...
            contents.push(' * ');
            contents.push(' * This file is automatically generated from the databases object settings in');
          ...
          contents.push(' * ');
          contents.push(' * This file is automatically generated from the "secrets" object settings in');
          contents.push(' * aquifer.secrets.json. If you need to change these settings, change them in');
          contents.push(' * that file and rebuild.');
          contents.push(' */\n');

          // Generate variables as php string.
          if (!_.isEmpty(secrets)) {
            _.forEach(secrets, function(value, key) {
              // First level keys should be converted to php variables.
              // If value is an object, generate a php array from it.
              if (_.isObject(value)) {
                if (!_.isEmpty(value)) {
                  contents.push('$' + key + ' = array(');
                  contents.push(jsonObjToPhpArray(value, 1));
                  contents.push(');\n');
                }
              }
              // Value not an object, generate a php integer, boolean value, or
              // string.
              else {
                contents.push('$' + key + ' = ' + renderPhpVar(value) + ';\n');
              }
            });
          }
