  ...
  create(destination, options) {
    return new Promise((resolve, reject) => {
      ...
      if (this.options.buildMethod == 'drush make') {
        ...
        commandOptions = {cwd: this.destination};
        afterBuild = () => {
          fs.rmdirSync(path.join(this.destination, 'sites/default/files'));
          glob.sync(path.join(this.destination, 'sites/default/*')).forEach((item, index, array) => {
            fs.unlinkSync(item);
          });
        };
      }
