  ...
  create(destination, options) {
    return new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        // Add config links.
        Object.keys(project.config.paths.configDirectories).forEach((key) => {
          if (key !== 'root' && key !== 'buildConfig') {
            links.push({
              src: project.config.paths.configDirectories[key],
              dest: path.join(project.config.paths.configDirectories.buildConfig, key),
              type: 'dir'
            });
          }
        });
