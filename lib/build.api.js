  ...
  create(destination, options) {
    return new Promise((resolve, reject) => {
      ...
      .then(() => {
        ...
        // Add config links.
        Object.keys(project.config.paths.config_directories).forEach((key) => {
          if (key !== 'root') {
            links.push({
              src: project.config.paths.config_directories[key],
              dest: path.join(project.config.paths.config_directories.buildconfig, key),
              type: 'dir'
            });
          }
        });
