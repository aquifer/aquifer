  ...
  initialize(directory, name, configJsonPath) {
    return new Promise((resolve, reject) => {
      ...
      this.absolutePaths = {
        ...
        config_directories: {
          root: path.join(directory, this.config.paths.config_directories.root),
          base: path.join(directory, this.config.paths.config_directories.base)
        },
        files: {
  ...
  create() {
    return new Promise((resolve, reject) => {
      ...
      if (this.drupalVersion == 8) {
        fs.mkdirSync(this.absolutePaths.config_directories.root);
        fs.mkdirSync(this.absolutePaths.config_directories.base);
      }
      ...
      if (this.drupalVersion == 8) {
        touch.sync(path.join(this.absolutePaths.config_directories.root, '.gitkeep'));
      }
      ...
      if (this.drupalVersion == 8) {
        fs.copySync(path.join(this.srcDir, 'config_directories.htaccess'), path.join(this.absolutePaths.config_directories.root, '.htaccess'));
        fs.copySync(path.join(this.srcDir, 'config_directories.htaccess'), path.join(this.absolutePaths.config_directories.base, '.htaccess'));
      }
