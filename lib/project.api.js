...
class Project {
  ...
  initialize(directory, name, configJsonPath) {
    return new Promise((resolve, reject) => {
      ...
      if (this.drupalVersion == 8) {
        this.absolutePaths.configDirectories = {
          root: path.join(directory, this.config.paths.configDirectories.root),
          base: path.join(directory, this.config.paths.configDirectories.base)
        };
      }
  ...
  create() {
    return new Promise((resolve, reject) => {
      ...
      if (this.drupalVersion == 8) {
        fs.mkdirSync(this.absolutePaths.configDirectories.root);
        fs.mkdirSync(this.absolutePaths.configDirectories.base);
      }
      ...
      if (this.drupalVersion == 8) {
        touch.sync(path.join(this.absolutePaths.configDirectories.root, '.gitkeep'));
      }
      ...
      if (this.drupalVersion == 8) {
        fs.copySync(path.join(this.srcDir, 'config.htaccess'), path.join(this.absolutePaths.configDirectories.root, '.htaccess'));
        fs.copySync(path.join(this.srcDir, 'config.htaccess'), path.join(this.absolutePaths.configDirectories.base, '.htaccess'));
      }
