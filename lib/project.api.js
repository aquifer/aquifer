...
module.exports = function (Aquifer) {
   ...
   * @param {string} drupalVeriosn major Drupal version to be used if not specified in aquifer.json.
   *
   * @returns {object} instance of the Project object.
   */
  var Project = function(directory, name, configJsonPath, drupalVersion) {
    var self          = this,
        _             = require('lodash'),
        fs            = require('fs-extra'),
        path          = require('path'),
    ...
    // If this is a new (uninitialized) project. Figure out the major Drupal
    // version, and then load the default json.
    else {
      srcDir = path.join(srcDir, 'd' + drupalVersion);
