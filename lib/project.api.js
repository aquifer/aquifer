...
class Project {
  ...
  constructor(Aquifer, drupalVersion) {
    ...
    this.paths = {};
    this.absolutePaths = {};
    this.drupalVersion = typeof drupalVersion === 'undefined' ? 7 : drupalVersion;
  ...
  initialize(directory, name, configJsonPath) {
    return new Promise((resolve, reject) => {
      ...
      this.drupalVersion = this.config.core;
      ...
      if (this.drupalVersion == '8') {
        // TODO: To make this configurable, we would have to add an option to
        // aquifer.json and add this directory to the settings.php that is
        // copied over.
        this.absolutePaths.drupalConfiguration = path.join(directory, 'drupal_configuration');
      }
  ...
  create() {
    return new Promise((resolve, reject) => {
      ...
      if (this.drupalVersion == '8') {
        fs.mkdirSync(this.absolutePaths.drupalConfiguration);
        fs.copySync(path.join(this.srcDir, 'core.extension.yml'), path.join(this.absolutePaths.drupalConfiguration, 'core.extension.yml'));
      }
